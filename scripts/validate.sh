#!/usr/bin/env bash
# Validate source formats, documentation, manifests, and Milestone 5 boundaries.
set -euo pipefail
...
for argument in "$@"; do
  case "${argument}" in
    --strict) strict=true ;;
    --promotion-context) promotion_context=true ;;
    *) echo 'Usage: validate.sh [--strict] [--promotion-context]' >&2; exit 64 ;;
  esac
done
missing=0
...
echo '==> YAML, JSON, structure, links, scope, package and role policy'
ruby scripts/validate_yaml.rb
python3 scripts/validate_repository.py
python3 scripts/check_markdown_links.py
python3 scripts/check_milestone5_scope.py
