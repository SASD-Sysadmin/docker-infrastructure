...
[[ "${branch}" == "${environment}" ]] || die 'branch and environment must match for the SASD r10k deployment' 64
...
[[ "${architecture}" == 'amd64' ]] || die "unsupported Puppet Server architecture ${architecture}; the current server implementation supports amd64" 69
...
# oneshot unit remains manual: the current design deliberately has no timer/webhook.
