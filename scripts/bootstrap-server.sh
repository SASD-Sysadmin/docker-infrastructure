...
[[ "${branch}" == "${environment}" ]] || die 'branch and environment must match for Milestone 6 r10k deployment' 64
...
[[ "${architecture}" == 'amd64' ]] || die "unsupported Puppet Server architecture ${architecture}; Milestone 6 supports amd64" 69
...
# oneshot unit remains manual: Milestone 6 deliberately has no timer/webhook.
