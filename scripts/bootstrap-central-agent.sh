...
Usage: bootstrap-central-agent.sh --server FQDN [OPTIONS]
  ...
  --package-source SRC   distribution (default) or puppet-core.
  --api-key-file FILE    Root-only API key file for puppet-core.
...
log "agent platform: ${ID} ${VERSION_ID}; certname: ${certname}"
...
# Refuse silent identity replacement. A certname change requires documented SSL
# cleanup on both agent and CA, not an in-place edit over existing key material.
...
set +e
bootstrap_output="$("${puppet_bin}" ssl bootstrap --waitforcert 0 2>&1)"
result=$?
set -e
printf '%s\n' "${bootstrap_output}"
if [[ "${result}" -eq 0 ]]; then
