#!/usr/bin/env python3
"""Enforce the reviewed Milestone 4 catalog resource boundary."""
        ...
        for path in sorted(base.rglob('*.pp')):
            ...
            for number,line in enumerate(text.splitlines(),1):
                ...
                if kind not in ALLOWED:
                    print(f'ERROR: {path.relative_to(ROOT)}:{number}: {kind} is outside Milestone 4 catalog scope',file=sys.stderr); failures+=1
            if re.search(r'^\s*exec\s*\{',text,re.M):
                if ALLOWED_EXEC_TITLE not in text or ALLOWED_EXEC_COMMAND not in text or 'refreshonly => true' not in text:
                    print(f'ERROR: {path.relative_to(ROOT)} contains a non-allowlisted or non-refresh-only exec',file=sys.stderr); failures+=1
    for kind in ALLOWED:
        if kind not in found:
            print(f'ERROR: required Milestone 4 resource type not exercised: {kind}',file=sys.stderr); failures+=1
    if failures: return 1
    print('Milestone 4 catalog boundary passed: package, file, service, and one refresh-only systemd reload exec.')
    return 0
