...
def main():
    ...
    if version!='0.8.0': fail('VERSION must be 0.8.0 for Milestone 8'); failures+=1
    ...
    for rel in ('config/role-catalog.json','config/node-data-contract.json','config/platform-catalog.json','config/operations-policy.json'):
        d=json.loads((ROOT/rel).read_text())
        if d.get('version')!=version: fail(f'{rel}: version differs from VERSION'); failures+=1
    ...
    print('Milestone 8 repository structure validation passed.'); return 0
