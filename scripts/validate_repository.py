...
def main():
    ...
    if version!='0.9.0': fail('VERSION must be 0.9.0 for Milestone 9'); failures+=1
    ...
    for rel in ('config/role-catalog.json','config/node-data-contract.json','config/platform-catalog.json','config/operations-policy.json','config/sdk-catalog.json'):
        d=json.loads((ROOT/rel).read_text())
        if d.get('version')!=version: fail(f'{rel}: version differs from VERSION'); failures+=1
    ...
    print('Milestone 9 repository structure validation passed.'); return 0
