...
required={'java_sdk','php_sdk','dotnet_sdk'}
if set(cat.get('profiles',{}))!=required: errors.append('SDK catalog must contain java_sdk, php_sdk, and dotnet_sdk')
java=cat.get('profiles',{}).get('java_sdk',{}); php=cat.get('profiles',{}).get('php_sdk',{}); dotnet=cat.get('profiles',{}).get('dotnet_sdk',{})
if java.get('java_major')!=17 or java.get('build_tool')!='maven': errors.append('Java contract must be OpenJDK 17 with Maven')
for name,meta in [('java_sdk',java),('php_sdk',php),('dotnet_sdk',dotnet)]:
...
if dotnet.get('dotnet_major')!=10 or dotnet.get('support_end')!='2028-11-14': errors.append('.NET contract must use .NET 10 LTS')
if dotnet.get('supported_architectures')!=['amd64','x86_64']: errors.append('.NET architecture allowlist mismatch')
for key in ('installs_workloads','installs_global_tools','manages_nuget_sources','manages_global_json'):
    if dotnet.get(key) is not False: errors.append(f'dotnet_sdk must set {key}=false')
...
for role,needed in {'java_development':'java_sdk','php_development':'php_sdk','polyglot_development':'java_sdk','dotnet_development':'dotnet_sdk'}.items():
...
print('SDK catalog passed: Java 17, distribution PHP, and .NET 10 LTS policies are internally consistent.')
