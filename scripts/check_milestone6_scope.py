#!/usr/bin/env python3
"""Enforce the reviewed Milestone 6 catalog resource boundary."""
...
def main():
        ...
        for path in sorted(base.rglob('*.pp')):
            ...
            for n,line in enumerate(text.splitlines(),1):
                ...
                if kind not in ALLOWED:
                    print(f'ERROR: {path.relative_to(ROOT)}:{n}: {kind} outside Milestone 6 scope',file=sys.stderr); failures+=1
    ...
    print('Milestone 6 catalog boundary passed: package, file, service, and one refresh-only systemd reload exec.')
    return 0
