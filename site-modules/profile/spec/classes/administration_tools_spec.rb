...
describe 'profile::administration_tools' do
  SUPPORTED_FACTS.each do |platform,platform_facts|
    context "on #{platform}" do
      ...
      PACKAGE_EXPECTATIONS.fetch(platform_facts[:os]['family'].to_sym).fetch(:administration_tools).each { |name| it { is_expected.to contain_package(name).with_ensure('installed') } }
    end
  end
end
