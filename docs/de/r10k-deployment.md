...
Das Skript verwendet eine Sperrdatei, deployed nur ein Environment, installiert Puppetfile-Abhängigkeiten, prüft die Manifeste und schreibt einen Zeitstempel. Es gibt auch in Milestone 4 bewusst keinen Deployment-Timer und keinen ungeprüften Webhook. Beim Server-Bootstrap wird eine stabile Kopie unter `/usr/local/libexec/sasd-puppet/` installiert. Zusätzlich wird die manuell aufzurufende One-shot-Unit `sasd-puppet-deploy.service` installiert:
